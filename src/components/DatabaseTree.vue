...
<template>
  <div class="tree" role="tree" aria-label="数据库对象树">
    ...
    <template v-for="db in serverExpanded ? visibleDatabases : []" :key="db.name">
      <button class="tree-node db-node" :class="{ active: selected === db.name && (!isPostgres || !selectedSchema) }" role="treeitem" :aria-expanded="expanded.has(db.name)" @click="emit('selectDatabase', db.name)" @contextmenu.prevent="emit('databaseMenu', $event, db)">
            ...
            <template v-if="expandedSchemas.has(schemaKey(db.name,itemSchema.name))">
              ...
              <template v-for="category in definitions" v-else :key="category.key">
                ...
                <template v-if="isCategoryOpen(db.name,itemSchema.name,category.key)">
                  <div v-for="item in filtered(contextObjects(db.name,itemSchema.name)[category.key])" :key="`${category.key}:${item.name}`" class="tree-node object-node schema-object-node" role="treeitem" tabindex="0" @click="emit('inspect',item,db.name,itemSchema.name)" @keydown.enter="category.key==='tables'?emit('openTable',item.name,db.name,itemSchema.name):emit('openObject',category.key,item,db.name,itemSchema.name)" @dblclick="category.key==='tables'?emit('openTable',item.name,db.name,itemSchema.name):emit('openObject',category.key,item,db.name,itemSchema.name)" @contextmenu.prevent="category.key==='tables'?emit('tableMenu',$event,item,db.name,itemSchema.name):emit('objectMenu',$event,category.key,item,db.name,itemSchema.name)">
                    <span class="tree-indent level-four" /><span class="twisty placeholder" /><Icon :icon="iconFor(category.key)" class="node-icon" /><span class="node-label">{{ item.name }}</span><button v-if="category.key==='tables'" class="row-action" title="设计表" @click.stop="emit('designTable',item.name,db.name,itemSchema.name)"><Icon icon="lucide:panel-top-open" /></button>
                  </div>
                </template>
              </template>
            </template>
        ...
        <template v-else>
          ...
          <template v-for="category in definitions" v-else :key="category.key">
            ...
            <template v-if="isCategoryOpen(db.name,'',category.key)">
              <div v-for="item in filtered(contextObjects(db.name)[category.key])" :key="`${category.key}:${item.name}`" class="tree-node object-node" role="treeitem" tabindex="0" @click="emit('inspect',item,db.name,'')" @keydown.enter="category.key==='tables'?emit('openTable',item.name,db.name,''):emit('openObject',category.key,item,db.name,'')" @dblclick="category.key==='tables'?emit('openTable',item.name,db.name,''):emit('openObject',category.key,item,db.name,'')" @contextmenu.prevent="category.key==='tables'?emit('tableMenu',$event,item,db.name,''):emit('objectMenu',$event,category.key,item,db.name,'')">
                <span class="tree-indent level-three" /><span class="twisty placeholder" /><Icon :icon="iconFor(category.key)" class="node-icon" /><span class="node-label">{{ item.name }}</span><button v-if="category.key==='tables'" class="row-action" title="设计表" @click.stop="emit('designTable',item.name,db.name,'')"><Icon icon="lucide:panel-top-open" /></button>
              </div>
            </template>
          </template>
        </template>
