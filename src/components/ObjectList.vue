...
<template>
  <section class="object-list-view">
    <header class="view-header"><div><Icon icon="lucide:folder-tree" /><span><strong>{{ tab.title }}</strong><small>{{ tab.database }}</small></span></div><label><Icon icon="lucide:search" /><input v-model="filter" aria-label="筛选对象" placeholder="筛选对象" /></label></header>
